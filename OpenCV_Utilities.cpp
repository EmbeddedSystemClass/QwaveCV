...
/*
EXTERN_C void QwaveCV_cvuiShow(lvError * errorHandle, CVImageHandle *hImage, uchar * windows_name) {
    CVERROR error = ERR_SUCCESS;
    QwaveCV_ReturnOnPreviousError(errorHandle);
    try
    {
...
EXTERN_C void QwaveCV_cannySetting(lvError * errorHandle, CVImageHandle *hImage, int *low_threshold, int *high_threshold, uint8_t *use_canny) {
    CVERROR error = ERR_SUCCESS;
    QwaveCV_ReturnOnPreviousError(errorHandle);
    try
    {
